			...
			<div class="product" v-for="(item,index) in row1">
				...
				<div class="info">
					...
					<div class="button" @click="addItem1(index)">
						 <div class="cartIcon" :style="{backgroundImage:image}"></div>
						<div>Add to cart</div>
					</div>
				</div>
...
import {store} from './store/store.js'
	export default{
		data:function(){
	return{
		image: store.state.imageStore[3].source
	}
	},
		computed:{
			row1: function(){
				return store.state.accessories[0]
			},
			row2: function(){
				return store.state.accessories[1]
			}
		...
		methods:{
			addItem1: function(index){
				store.state.counter++;
				store.state.totalPrice +=store.state.accessories[0][index].numPrice;
				
			},
			addItem2: function(indox){
				store.state.counter++;
				store.state.totalPrice +=store.state.accessories[1][indox].numPrice;
